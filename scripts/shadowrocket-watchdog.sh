...
probe_url() {
  ...
  /usr/bin/curl -fsSI \
    ...
    --max-time "$MAX_TIME" \
    "$probe_target" >/dev/null
}

read_number() {
  number_file=$1
  number_fallback=$2
  number_value=$(cat "$number_file" 2>/dev/null || true)
...
run_watchdog() {
  ...
  if [ $((now - last_trigger)) -lt "$COOLDOWN_SECONDS" ]; then
    remaining=$((COOLDOWN_SECONDS - (now - last_trigger)))
    log "cooldown_active skip_trigger remaining=$remaining"
    return 0
  fi
